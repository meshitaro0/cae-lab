...
ここで比較する累積量は、各区間をN分割した最終値を $\alpha_N$、640分割した最終値を $\alpha_{640}$ とすると、**差の絶対値 $|\alpha_N-\alpha_{640}|$** である。「絶対差」という語だけで、符号付き差 $\alpha_N-\alpha_{640}$ や正規化しない差との区別を要求しない。相対差を求める場合は、分母に使う基準量と、基準量がゼロの場合の扱いも別途指定する。
...
演習6の表で指定された「同一積分点・同一時刻」は既知条件として使ってよく、小問1ではその条件の再確認を要求しない。小問4の意図は、表とは独立した実務データの一般的なレビューであるが、初稿ではこの切替が明示されていなかった。EPS等の出力量の定義も、「材料則・出力位置・履歴」の三観点から必須項目として推測させるべきではない。初稿答案の評価ではこの曖昧さを減点理由にせず、今後の一般レビュー設問では「材料モデル」「出力量の定義」「出力位置・時刻・処理」「負荷・初期化履歴」を明記する。提出済みNotebookの問題文・答案は保持する。
